...
export class PrivyEVMRelay implements IEVMHTLC {
  ...
  constructor(
    url: string | Url,
    client: PrivyEthereumEmbeddedWalletAccount,
    auth: IAuth,
  ) {
  ...
  private async _initiateOnErc20HTLC(
    ...
    try {
      ...
      const approval = await checkAllowanceAndApprove(
        ...
        tokenAddress,
        htlcAddress,
        this.wallet as any, // Type assertion for compatibility
      );
  ...
  private encodeInitiateCall(
    ...
    // This is a simplified encoding - you'll need to implement proper ABI encoding
    // based on your HTLC contract's initiate function signature
    // For now, returning a placeholder
    return (
      '0x' +
      'initiate' +
      redeemer.slice(2) +
      timelock.toString(16).padStart(64, '0') +
